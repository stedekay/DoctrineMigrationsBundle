...
.. tip::
    When you are using ACL, you have to be careful with the autogenerated migration classes,
    because they will be dropped with the next migration. So you have to remove the drop table
 	commands for each acl_* table in the up()-method. The same applies for all create table
 	commands for each acl_* table in the down()-method. Alternatively it is possible to move all
    acl_* tables to another database connection, then they will be ignored when running the 
